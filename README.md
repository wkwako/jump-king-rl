...
| Speed reward | On screen completion; higher reward for completing a screen in fewer actions | 100/num_actions | all
...
An initial attempt to train pure RL under BC+RL's reward scheme produced no learning at all. Without demonstration guidance, the base reward was too sparse to yield a usable gradient. Pure RL therefore required two additional shaping signals to learn: proximity-to-goal, which rewards movement toward the goal along a platform, and a broadened new-platform reward, which rewards arrival on any new platform rather than only goal-directed ones. This asymmetry is recorded in the Scope column of the above table, and it runs in RL's favor: pure RL was given strictly more reward guidance than BC+RL, so its failures cannot be attributed to receiving a sparser signal.
...
Other screens occasionally used different hyperparameters. For example, on wind screens (25-31), an ent\_coef of 0.25 was used to encourage exploration due to no-op class domination. 
...
- Screen 10 - Easy: has an action space size of 10 and very wide jump windows.
- Screen 39 - Medium: has an action space size of 12, nearly all jumps are non-maximum, and have moderately-wide jump windows.
- Screen 37 - Hard: has an action space size of 10, icy platforms, most jumps are non-maximum, and has the trickiest jump in the game that requires a jump while the player is sliding from a previous jump.
...
3. **RL-only models are unable to learn many screens (8, 16, 26, 36) due to a combination of sparse rewards, long time horizons, and rarity of picking the correct action.** Pure RL converged on screens 0 and 1, with reward still rising on screen 3 after ~330k time steps. Every screen with a structural or environmental complication - downward progression (8), sub-0.1s precision (16), wind (26), ice (36) - defeated it, under identical action spaces to BC+RL. Screen 36 shows the failure explicitly: mean reward rose from ~-114 to -60 over ~80k timesteps before plateauing, mirroring screen 8, and even discovered the correct ice technique, but the learning stalled short of a solution. Under greedy evaluation it reached the second-to-last platform in all 500 episodes, then failed the final jump in all 500. The terminal jump never became the highest-probability action, so it played through the level perfectly, then failed at the same spot every run. The terminal states were reached too rarely during training for their reward to consolidate. This is the failure BC+RL repairs: demonstration initialization places the agent at the frontier densely enough to reinforce those final jumps, clearing the screen 84.4% of the time.
...
4. **Standard cross-entropy loss fails on screens with severely imbalanced action distributions.** On wind screens, for example, no-ops constitute 95% of the action space. Jumps are necessary to complete each wind screen, but standard loss functions cause no-ops to dominate the learning space. I used a class-weighted loss function that inversely weights actions by frequency, which keeps rare actions learnable. 
...
I created a reinforcement learning agent to play and complete Jump King, from creating a full pipeline directly interfacing with the live game via reflection-based state extraction and TCP socket communication, to a custom Gymnasium environment. Agents can reliably complete all 43 screens, and the first full game completion was achieved in 9:00.855 minutes, with 256 jumps and 3 falls. BC and RL are complementary: BC provides a near-correct starting policy almost for free, and RL then refines it and handles what human demonstrations miss. The hybrid model was able to learn every screen in the game. For Jump King, state representation can matter more than hyperparameters, as evidenced by a single state change - y to y%360 - tripling BC accuracy. Jump King was a useful controlled test bed where the same task can be completed by several different models and compared with minimal model differences. A formal paper is in progress, targeting AIIDE 2027 submission. Future work includes targeting Jump King DLC maps for a harder challenge.
